...
1. 基类信号 `RACSignal`   
  ...
  通过方法 `createSignal` 创建信号，传入一个 block1 ，在进行信号订阅时，使用方法 `subscribeNext` 进行消息的接收，但此时 `RACSignal ` 还是冷信号，需要在 block1 里面，使用 `RACSubscriber` 对象进行发送信号，我们称之为 `订阅信号`，方法为 `sendNext ` ，才可以使信号变成热信号，也就是可以接收到消息。        
